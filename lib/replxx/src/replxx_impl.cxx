...
void Replxx::ReplxxImpl::render( HINT_ACTION hintAction_ ) {
	...
	if ( pi.index != -1 ) {
		...
		if ( nearby >= 0 && nearby < colors.size() ) {
			colors[nearby] = pi.error ? ERROR : Replxx::Color::BRIGHTRED;
		}
	}
...
Replxx::ReplxxImpl::paren_info_t Replxx::ReplxxImpl::matching_paren( void ) {
	int pos = _pos;
	if (pos >= _data.length()) {
		if (pos > 0 && strchr(")]}", _data[pos - 1])) {
			pos = pos - 1;
		} else {
			return ( paren_info_t{ -1, false } );
		}
	} else if (pos > 0 && !strchr("()[]{}", _data[pos]) && strchr(")]}", _data[pos - 1])) {
		pos = pos - 1;
	}
	/* this scans for a brace matching _data[pos] to highlight */
	...
	for (int i = pos + scanDirection; i >= 0 && i < _data.length(); i += scanDirection) {
